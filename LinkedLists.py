...
#cập nhật self._head để trỏ đến newest và newest trở thành nút đầu tiên của danh sách, và danh sách được mở rộng ra phía trước.
    ...
    def search (self , key ):
        ...
        while p : # vòng lặp while để duyệt qua danh sách (list)
            if p.element== key : # kiểm tra xem giá trị của nút(node) hiện tại có bằng giá trị key hay không
                return index # tìm thấy key thì trả về vị trí của key
    ...
    def insertsorted(self,e): # tạo function chèn và sắp xếp
        newest = _Node(e, None) # tạo node newest
        if self.isempty(): # kiêm tra nếu danh sách trống thì giá trị  chèn vào là giá trị đầu tiên 
            self._head = newest 
        else:  # ngược lại đặt p và q ở head 
            p=self._head
            q=self._head
            while p and p.element<e: # vòng lặp while thực hiện khi tồn tại p và giá trị tại p < giá trị cần chèn
                q=p
                p=p.next
            if p == self._head : # kt giá trị p ở đầu danh sách liên kết 
                newest.next = self._head
                self._head = newest 
            else: # 
                newest.next = q.next
